...
export default function AdminUsers() {
  ...
  useEffect(() => {
    api
      ...
      .catch((err) => console.error(t("admin_users_page_error_loading"), err));
  }, []);
  ...
  const confirmDelete = async () => {
    if (userToDelete) {
      ...
      setUsers((prev) =>
        prev.filter((t) => t.user_id !== userToDelete.user_id)
      );
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">{t("admin_users_page_title")}</h1>
            <p className="text-gray-400 mt-1">{t("admin_users_page_subtitle")}</p>
          </div>
        </div>
        <CreateButton onClick={() => navigate("/admin/users/new")}>
          {t("admin_users_page_create_button")}
        </CreateButton>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {users.map((u) => (
          <div
            key={u.user_id}
            className="group relative overflow-hidden bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 hover:bg-white/10 hover:border-white/20 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-500/5"
          >
            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-purple-500/5 to-violet-500/5 rounded-full -mr-16 -mt-16 group-hover:from-purple-500/10 group-hover:to-violet-500/10 transition-all duration-300"></div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-emerald-500/20 text-emerald-400 border border-emerald-500/20">
                      {u.role === "ADMIN" ? "👑 " + t("admin_page_role_admin") : "👤 " + t("admin_page_role_user")}
                    </span>
                  </div>
                </div>
              </div>
      ...
      {users.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-400 mb-2">
            {t("admin_users_page_no_users_title")}
          </h3>
          <p className="text-gray-500 mb-6">
            {t("admin_users_page_no_users_message")}
          </p>
          <CreateButton onClick={() => navigate("/admin/users/new")}>
          {t("admin_users_page_create_button")}
        </CreateButton>
          ...
          {userToDelete && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-6">
              <div className="text-sm text-gray-300 mb-2">
                <span className="font-medium text-white">{t("admin_page_name")}</span>{" "}
                {userToDelete.name}
              </div>
              <div className="text-sm text-gray-300">
                <span className="font-medium text-white">{t("admin_page_email")}</span>{" "}
                {userToDelete.email}
              </div>
            </div>
          )}
